web_debug:
  ...
  webget_response:
    - announce to_console "<&3>-- <queue.script.name||invalid> - WebGet_Response ------"
  ...
  web_request:
    ...
    - define content <[content].include_single[<&6><&lt>context<&6>.<&e>headers<&6><&gt> <&b>| <&3><context.headers.to_yaml.replace_text[-].with[<&e>- <&3>].replace_text[<&co>].with[<&e><&co> <&3>].if_null[<&c>invalid]> <&b>| <&a>returns a MapTag of all input headers, where the key is the header name and the value is a ListTag of header values for that name.]>
