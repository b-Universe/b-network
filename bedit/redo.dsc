bedit_redo_command:
  ...
  name: bredo
  usage: /bredo
  description: Redos bEdit actions made
  script:
    # Stop if they don't have anything to undo
    - if !<player.flag[behr.essentials.bedit.history].is_truthy>:
      - define reason "You don't have anything to redo"
      - inject command_error

    # Check for anything left to undo
    - define times <player.flag[behr.essentials.bedit.history].filter_tag[<[filter_value.action].is_in[set|undo]>].keys>
    - if <[times].is_empty>:
      - define reason "You don't have anything left to redo"
      - inject command_error
    ...
    - foreach <player.flag[behr.essentials.bedit.history.<[time]>.block]> key:location as:new_material:
      ...
      - flag <[location]> behr.essentials.bedit.history.<[time]>.action:redo expire:1d
